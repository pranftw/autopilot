# {name} -- Agent Operations Guide

You are an external coding agent working on the **{name}** project, powered by the
...
autopilot execute -c 'from {name}.module import ...; result = m.forward(batch); print(result)'

# check what parameters exist
autopilot execute -c 'from {name}.module import ...; print(list(m.parameters()))'
```
...
Validated import paths for the most-used framework types:
...
from autopilot.core.config import Config, AutoPilotConfig
from autopilot.core.experiment import Experiment
from autopilot.core.forest import Forest
from autopilot.core.query import QueryBuilder
from autopilot.core.comparison import MetricsComparator, ComparatorMetric, Delta
from autopilot.core.metric_utils import infer_direction
from autopilot.core.context import ContextEntry, ContextLog
from autopilot.core.types import EvalDatum, Datum
from autopilot.core.scheduler import Scheduler, LambdaScheduler
...
from autopilot.policy.quality_first import QualityFirstPolicy
from autopilot.policy.gates import MinGate, MaxGate, RangeGate, CustomGate, MonotonicGate, BudgetGate
from autopilot.data.dataset import Dataset
from autopilot.data.datamodule import DataModule, Stage
from autopilot.data.dataloader import DataLoader
from autopilot.tracking.io import utc_now_iso, parse_timestamp
```

## Metric Key Naming

Trainer merges train/val metrics with prefixed keys when both splits exist:

- Both splits present: keys become `train_*` and `val_*` (e.g. `train_accuracy`, `val_accuracy`)
- Only validation metrics: stored unprefixed (e.g. `accuracy`)
- Only training metrics: stored unprefixed

When writing `--metric-gt`, `--sort`, `--best`, or policy gate thresholds, use
the **prefixed** key that Trainer actually stores (e.g. `val_accuracy`, not `accuracy`).

## Module Attribute Assignment Trap

When subclassing `Module`, assigning a `Parameter` to `self` registers it:

```python
class MyModule(AutoPilotModule):
  def __init__(self):
    super().__init__()
    self.prompt = PathParameter(source=Path('prompt.txt'), pattern='*.txt')
```

But reassigning later (`self.prompt = 'new string'`) replaces the Parameter object
with a plain string -- the parameter is lost from `module.parameters()`. Always
mutate parameter values through the Parameter API (e.g. `self.prompt.value = ...`
for ScalarParameter), never reassign the attribute itself.

## EvalDatum vs Datum

When `batch_size > 1`, `training_step` receives a `Datum` whose `items` contains
one `EvalDatum` per sample: `Datum(items=[EvalDatum, EvalDatum, ...])`.

Always iterate `datum.items` for per-sample processing. Do not assume the top-level
object is a single `EvalDatum` with scalar fields.

## Autograd Graph Preservation

Call modules as `module(datum)` or `self(datum)`, NOT `module.forward(datum)`.
`Module.__call__` wraps `forward()` and records a `ModuleCallOperator` node in the
autograd graph. Calling `forward()` directly bypasses graph recording.

## Git-to-Store Glossary

| Git concept | AutoPilot Store equivalent |
|-------------|---------------------------|
| repository | `FileStore` (content-addressed blob store) |
| branch | experiment branch (`Store.branch(experiment_id)`) |
| commit | snapshot epoch (`Store.snapshot()`) |
| HEAD | `Store.head` (active experiment id) |
| checkout | `Store.checkout(experiment_id, epoch)` |
| tag | `Store.tag(name, experiment_id, epoch)` |
| stash | `Store.stash()` / `Store.stash_pop()` |
| reflog | `FileStore.iter_reflog()` |
| merge | three-step: `merge_analysis` -> `merge_preview` -> `merge_apply` |
| worktree | `IsolatedEnvironment` (per-experiment directory) |

## Store Bootstrap Sequence

When starting a new experiment with store versioning:

1. Create `AutoPilotConfig` with workspace path
2. Create `FileStore(config)` and register parameters: `store.register_parameters({{'prompt': param}})`
3. Create experiment in the forest (`tree.add(Node(...))`)
4. Branch the store: `store.branch(experiment_id)`
5. Take initial snapshot: `store.snapshot(experiment_id, context='initial state')`
6. Now Trainer can use `store=store` for epoch-level versioning

## Worktree Isolation Semantics

`PathParameter.bind(root)` / `unbind()` switch between a worktree copy and the
canonical source. Trainer manages bind/unbind lifecycle automatically when an
`IsolatedEnvironment` is configured.

Caveats:
- `tree switch` auto-checkouts by default. Pass `--no-checkout` to skip disk sync.
- `PathParameter.restore()` writes text content only; executable permissions are NOT preserved.
- Binary files are skipped during snapshot and protected from deletion on checkout.
...
- `PathParameter(source=Path('path'))` -- source is a Path object
- `ComparatorMetric(metric_name='accuracy', higher_is_better=True)` -- keyword `metric_name`
- `infer_direction` lives in `autopilot.core.metric_utils`, NOT `core.comparison`
- Gates live in `autopilot.policy.gates` (plural), NOT `autopilot.policy.gate`
- `QueryBuilder` chain methods are immutable -- reassign: `b = b.metric_gt(...)`
- `QueryBuilder` status filters: `.completed()`, `.pending()`, `.failed()`, `.running()`, `.cancelled()`
- `QueryBuilder` ordering: `.order_by_metric(name)` (not `.sort_by()`)
- `QueryBuilder` materialization: `.all()` returns `list[Node]`
- `FileForest(store=file_store)` -- constructor takes a FileStore, not config
- `FileStore(config)` then `store.register_parameters({{'name': param}})`
