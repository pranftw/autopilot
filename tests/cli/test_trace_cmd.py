...
class TestTraceCommand:
  ...
  def test_collect_happy_path(self, tmp_path, capsys):
    ...
    append_epoch_artifact(exp_dir, 1, 'data.jsonl', {'item_id': 'a', 'success': True})
    append_epoch_artifact(exp_dir, 1, 'data.jsonl', {'item_id': 'b', 'success': False})
  ...
  def test_collect_with_limit(self, tmp_path, capsys):
    ...
    exp_dir = tmp_path / 'test-exp'
    for i in range(5):
      append_epoch_artifact(exp_dir, 1, 'data.jsonl', {'item_id': str(i)})
  ...
  def test_inspect_filters_by_item_id(self, tmp_path, capsys):
    ctx = _make_ctx(tmp_path)
    exp_dir = tmp_path / 'test-exp'
    ...
    assert r['count'] == 1
    assert r['matches'][0]['item_id'] == 'node_b'
  ...
  def test_inspect_no_matches(self, tmp_path, capsys):
    ctx = _make_ctx(tmp_path)
    exp_dir = tmp_path / 'test-exp'
    append_epoch_artifact(exp_dir, 1, 'data.jsonl', {'item_id': 'other'})
